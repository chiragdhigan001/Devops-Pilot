...
import { generateDockerfile } from './aiService.js';
...
const detectTechStack = async (workspacePath) => {
  const files = await fs.readdir(workspacePath);
  if (files.includes('package.json')) {
    const pkg = JSON.parse(await fs.readFile(path.join(workspacePath, 'package.json'), 'utf8'));
    const deps = { ...pkg.dependencies, ...pkg.devDependencies };
    if (deps.next) return { stack: 'Next.js', port: 3000 };
    if (deps.vite) return { stack: 'Vite React', port: 5173 };
    if (deps.react || deps['react-dom']) return { stack: 'React', port: 3000 };
    if (deps.express) return { stack: 'Express', port: 5000 };
    if (deps.vue) return { stack: 'Vue', port: 3000 };
    if (deps.angular || deps['@angular/core']) return { stack: 'Angular', port: 3000 };
    return { stack: 'Node.js', port: 3000 };
  }
  if (files.includes('requirements.txt')) return { stack: 'Python', port: 5000 };
  if (files.includes('pyproject.toml')) return { stack: 'Python', port: 5000 };
  if (files.includes('go.mod')) return { stack: 'Go', port: 8080 };
  if (files.includes('Cargo.toml')) return { stack: 'Rust', port: 8080 };
  if (files.some(f => f.endsWith('.html'))) return { stack: 'HTML Static', port: 80 };
  return { stack: 'Node.js', port: 3000 };
};

const ensureDockerfile = async (workspacePath, onLog, githubToken) => {
  const dockerfilePath = path.join(workspacePath, 'Dockerfile');
  try {
    await fs.access(dockerfilePath);
    onLog('Dockerfile found in repository');
    return dockerfilePath;
  } catch {
    onLog('No Dockerfile found. Auto-generating one...');
    const { stack, port } = await detectTechStack(workspacePath);
    onLog(`Detected tech stack: ${stack}, using port ${port}`);
    const dockerfile = await generateDockerfile(stack, port);
    await fs.writeFile(dockerfilePath, dockerfile, 'utf8');
    onLog(`Dockerfile generated for ${stack}`);

    try {
      await runCommand({ command: 'git', args: ['add', 'Dockerfile'], cwd: workspacePath, onStdout: () => {}, onStderr: () => {} });
      await runCommand({ command: 'git', args: ['commit', '-m', 'Add Dockerfile (auto-generated by DevOpsPilot AI)'], cwd: workspacePath, onStdout: () => {}, onStderr: () => {} });
      if (githubToken) {
        await runCommand({ command: 'git', args: ['push'], cwd: workspacePath, onStdout: () => {}, onStderr: () => {} });
        onLog('Dockerfile committed and pushed to repository');
      } else {
        onLog('Dockerfile committed locally (no GitHub token for push)');
      }
    } catch (err) {
      onLog('Warning: Could not commit/push Dockerfile to repository');
    }
...
export const buildAndRunDeployment = async ({
  ...
  try {
    ...
    await ensureDockerfile(context.workspacePath, onLog, githubToken);
